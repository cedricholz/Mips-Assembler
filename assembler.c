...
int pass_one(FILE *input, FILE *output, SymbolTable *symtbl) {
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        ...
        if (token) {
            int label = add_if_label(input_line, token, byte_offset,
                                     symtbl); //Scan for labels                                                  //If label is valid get next token
            if (label) {                                                  //take next token regardless of validity
                token = strtok(NULL, IGNORE_CHARS);
            }

            if (token) {
                if (label < 0) {                                            //Error recording
                    ret_code = label;
                }
                ...
                char *args[MAX_ARGS];
                int num_args = 0;
                ret_code = parse_args(input_line, args, &num_args);         //populate args[]
                if (ret_code != -1 && num_args > 0) {
...
int pass_two(FILE *input, FILE *output, SymbolTable *symtbl, SymbolTable *reltbl) {
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        ...
        if (name) {
            ...
            char *args[MAX_ARGS];
            int num_args = 0;
            char *this_arg = strtok(NULL, IGNORE_CHARS);
            while (this_arg != NULL) {
                args[num_args] = this_arg;
                num_args++;
                this_arg = strtok(NULL, IGNORE_CHARS);
            }
            /* Use translate_inst() to translate the instruction and write to output file.
               ...
               raise_inst_error(). */
            int instruction_code = translate_inst(output, name, args, num_args,
                                                  byte_offset, symtbl, reltbl);
            if (instruction_code) {
                raise_inst_error(input_line, name, args, num_args);
                ret_code = instruction_code;
            }
            byte_offset += 4;
        }
    }
